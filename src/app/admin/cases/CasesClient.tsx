...
import { CASE_IMAGES_MAX, cn } from '@/lib/utils';
...
type Form = {
  title: string; description: string; logo: string | null; logoRatio: number | null;
  images: string[]; instagramUrl: string; published: boolean;
};

const EMPTY: Form = { title: '', description: '', logo: null, logoRatio: null, images: [], instagramUrl: '', published: true };

/**
 * Пропорции логотипа (ширина / высота). Сайт по ним сразу решает, растянуть
 * логотип на всю карточку или вписать целиком, — без мелькания при загрузке.
 */
...
export function CasesClient({ initial }: { initial: Case[] }) {
  ...
  const setLogo = async (logo: string | null) => {
    setForm((f) => ({ ...f, logo, logoRatio: null }));
    if (!logo) return;
    const logoRatio = await measureRatio(logo);
    // Логотип могли успеть сменить, пока измеряли старый.
    setForm((f) => (f.logo === logo ? { ...f, logoRatio } : f));
  };
  ...
  const open = (c?: Case) => {
    ...
    setForm(
      c
        ? {
            title: c.title, description: c.description, logo: c.logo, logoRatio: c.logoRatio,
            images: c.images, instagramUrl: c.instagramUrl ?? '', published: c.published,
          }
        : EMPTY,
    );
    // У старых кейсов пропорции не сохранены — досчитываем при открытии.
    if (c?.logo && !c.logoRatio) {
      const logo = c.logo;
      measureRatio(logo).then((logoRatio) => setForm((f) => (f.logo === logo ? { ...f, logoRatio } : f)));
    }
  ...
  const save = async () => {
    ...
    if (uploading) return toast.error('Дождитесь, пока загрузятся фото');
    setBusy(true);
    try {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        eyebrow="Cases"
        title={<>Кейсы</>}
        subtitle={`Логотип, название, описание, фото-доказательства и ссылка на Instagram. На сайте сейчас ${live} из ${items.length}.`}
        action={
          editing === null && (
      ...
      {editing !== null && (
        // key: при переключении кейса форма пересоздаётся, и незаконченная
        // загрузка фото прошлого кейса отбрасывается, а не попадает в новый.
        <div key={editing} className="rounded-3xl border border-brand-lime/20 bg-white/[0.02] p-5 sm:p-6">
          <p className="mb-5 text-[10px] uppercase tracking-[0.24em] text-brand-orange">
            {editing === 'new' ? 'Новый кейс' : 'Редактирование'}
          </p>
          <div className="grid gap-5 md:grid-cols-[240px_1fr]">
            <ImageUpload label="Логотип" value={form.logo} onChange={setLogo} />
          ...
          <div className="mt-6 border-t border-white/[0.06] pt-5">
            <GalleryUpload
              label={`Фото-доказательства${form.images.length ? ` · ${form.images.length}` : ''} — скриншоты результатов, до и после, отзывы`}
              value={form.images}
              max={CASE_IMAGES_MAX}
            ...
            <p className="mt-2 text-[12px] text-light/45">
              Можно выбрать сразу несколько файлов, до 5 МБ каждый и не больше {CASE_IMAGES_MAX} в кейсе. На сайте они откроются галереей на странице кейса,
              первые три будут видны прямо на карточке.
            </p>
          </div>
          <div className="mt-6 flex justify-end gap-2">
            <button onClick={cancel} className="btn-ghost !px-5 !py-2 !text-[12px]">Отмена</button>
            <button onClick={save} disabled={busy || uploading} className="btn-lime !px-5 !py-2 !text-[12px] disabled:opacity-50">
              {busy ? 'Сохраняем…' : uploading ? 'Грузим фото…' : editing === 'new' ? 'Добавить кейс' : 'Сохранить'}
            </button>
          </div>
        </div>
      )}
            ...
            >
              ...
              <div className="mt-4 flex-1">
                ...
                <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-[12px]">
                  {c.images.length > 0 && <span className="text-light/55">{c.images.length} фото-доказательств</span>}
                  {c.instagramUrl && (
                    <a href={c.instagramUrl} target="_blank" rel="noreferrer" className="text-brand-lime hover:underline">
                      Instagram →
                    </a>
                  )}
                </div>
